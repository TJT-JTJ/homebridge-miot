...
class MiCloud {
  ...
  async login(username, password, options = {}) {
    ...
    this._cookieJar = {};
    const loginData = options.loginData || await this._loginStep1();
    this._loginData = loginData;
    const {
      ssecurity,
      userId,
      location
    } = await this._loginStep2(username, password, loginData, options);
    const {
      serviceToken
    } = await this._loginStep3(location);
  ...
  // --- private stuff ---
  async _requestUnencrypted(path, data, attempt = 0) {
    if (!this.isLoggedIn()) {
    ...
    if (res.status === 401) {
      return await this._retryAfterUnauthorized(path, data, attempt, false, responseText);
    }
    ...
    if (json && json.code === 3) {
      return await this._retryAfterUnauthorized(path, data, attempt, false, json);
    }
  ...
  async _requestEncrypted(path, data, attempt = 0) {
    if (!this.isLoggedIn()) {
      throw new Error('You are not logged in! Cannot make a request!');
    }
    const url = this._getApiUrl(this.country) + path;
    ...
    if (res.status === 401) {
      return await this._retryAfterUnauthorized(path, data, attempt, true, responseText);
    }
    ...
    const json = this._parseEncryptedResponse(signedNonce, responseText);
    if (json && json.code === 3) {
      return await this._retryAfterUnauthorized(path, data, attempt, true, json);
    }
  ...
  async _loginStep1() {
    const url = `https://account.xiaomi.com/pass/serviceLogin?sid=${this.sid}&_json=true`;
    const res = await fetch(url, {
      headers: {
        'User-Agent': this.USERAGENT,
        Cookie: this._cookieHeader(),
  ...
  async _loginStep2(username, password, loginData, options = {}) {
    const formPayload = {
      hash: crypto
        .createHash('md5')
        .update(password)
        ...
        .toUpperCase(),
      _json: 'true',
      sid: loginData.sid || this.sid,
      callback: loginData.callback || '',
      qs: loginData.qs || '',
      _sign: loginData.sign,
      user: username,
    };

    if (options.captcha) {
      formPayload.captCode = options.captcha;
    }

    const formData = querystring.stringify(formPayload);
    ...
    const res = await fetch(url, {
      ...
      headers: {
        'User-Agent': this.USERAGENT,
        'Content-Type': 'application/x-www-form-urlencoded',
        Cookie: this._cookieHeader(options.ick ? [`ick=${options.ick}`] : []),
      },
    });
    ...
    if (!ssecurity && captchaUrl) {
      const captchaData = await this._fetchCaptcha(captchaUrl);
      this._loginData = loginData;
      throw new Errors.CaptchaRequired(captchaData);
    }
    ...
    if ((loginData.sid || this.sid) !== 'xiaomiio' && nonce) {
      const sign = crypto.createHash('sha1').update(`nonce=${nonce}&${ssecurity}`).digest();
      const clientSign = sign.toString('base64');
      redirectLocation += `&clientSign=${encodeURIComponent(clientSign)}`;
    }
    return {
      ssecurity,
      userId,
  ...
  async _loginStep3(location) {
    ...
    const serviceToken = this._cookieJar.serviceToken;
    if (!serviceToken && res.headers && typeof res.headers.raw === 'function') {
      ...
      cookies.forEach(cookieStr => {
        ...
        const value = cookie.substr(idx + 1, cookie.length).trim();
        if (key === 'serviceToken') {
          this._cookieJar.serviceToken = value;
        }
      });
    }
    if (!this._cookieJar.serviceToken) {
      throw new Error('Login step 3 failed');
    }
    return {
      serviceToken: this._cookieJar.serviceToken,
    };
  }
  ...
  _apiCookieHeader() {
    const timezoneInfo = this._getTimezoneInfo();
    return [
      'sdkVersion=accountsdk-18.8.15',
      `deviceId=${this.CLIENT_ID}`,
      `userId=${this.userId}`,
      ...
      `is_daylight=${timezoneInfo.isDaylight}`,
      `dst_offset=${timezoneInfo.dstOffset}`,
  ...
  _getTimezoneInfo() {
    ...
    const isDaylight = this._isDaylightSavingTime() ? 1 : 0;
    const dstOffset = isDaylight ? 60 * 60 * 1000 : 0;
    return {
      timezone,
      isDaylight,
      dstOffset
  ...
  _parseEncryptedResponse(signedNonce, responseText) {
    ...
    try {
      ...
      this.logger.debug(`(MiCloud) Error decrypting response, attempting JSON parse: ${e.message}`);
      return JSON.parse(trimmed);
    }
  }

  async _retryAfterUnauthorized(path, data, attempt, encrypted, response) {
    this.logger.debug(`(MiCloud) Unauthorized response from MiCloud, will attempt relogin. Response: ${JSON.stringify(response)}`);
    this.serviceToken = null;
    this.ssecurity = null;
    this.userId = null;
    if (attempt >= 1 || !this.username || !this.password) {
      throw new Error('Unauthorized response from MiCloud. Unable to relogin.');
    }
    await this.login(this.username, this.password);
    if (encrypted) {
      return await this._requestEncrypted(path, data, attempt + 1);
    }
    return await this._requestUnencrypted(path, data, attempt + 1);
  }

  async _fetchCaptcha(captchaUrl) {
    const url = captchaUrl.startsWith('http') ? captchaUrl : `https://account.xiaomi.com${captchaUrl}`;
    const res = await fetch(url, {
      headers: {
        'User-Agent': this.USERAGENT,
        Cookie: this._cookieHeader(),
